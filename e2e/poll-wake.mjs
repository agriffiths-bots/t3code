...
const turnCount = () =>
  db.prepare(`SELECT COUNT(*) n FROM projection_turns WHERE thread_id=?`).get(root).n;
const userMsgs = () =>
  db
    .prepare(
      `SELECT message_id, turn_id, role, text, created_at FROM projection_thread_messages WHERE thread_id=? AND role='user' ORDER BY created_at ASC`,
    )
    .all(root);
