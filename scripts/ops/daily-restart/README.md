...
validation. A valid post-stop manifest is the one used for resume injection, so
a pending request that was picked up while shutdown was in progress is not
replayed as a new turn; if the post-stop refresh fails, the unchanged service is
restarted and the validated pre-stop manifest is injected instead.
