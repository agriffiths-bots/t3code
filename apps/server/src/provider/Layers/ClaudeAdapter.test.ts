...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("keeps a turn open across Claude background task notification results", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-task",
        uuid: "assistant-task-waiting",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-task-waiting",
          content: [{ type: "text", text: "Waiting for the command to finish." }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        is_error: false,
        errors: [],
        result: "Waiting for the command to finish.",
        session_id: "sdk-session-task",
        uuid: "result-task-intermediate",
      } as unknown as SDKMessage);
      ...
      const assistantTexts = runtimeEvents
        .filter((event) => event.type === "content.delta")
        .map((event) => event.payload.delta);
      assert.deepEqual(assistantTexts, ["Waiting for the command to finish."]);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("completes a deferred Claude result after runtime tasks drain and stream exits", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      harness.query.emit({
        type: "system",
        subtype: "task_started",
        session_id: "sdk-session-task-deferred",
        uuid: "task-started-deferred",
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-deferred",
        uuid: "task-notification-deferred",
        task_id: "task-deferred",
        tool_use_id: "tool-deferred",
        status: "completed",
        summary: "Background command completed",
  ...
  it.effect("completes a detached active turn from an owned task-notification result", () => {
    ...
    return Effect.gen(function* () {
      ...
      const turn = yield* adapter.sendTurn({
        threadId: session.threadId,
        input: "finish from a task notification",
        attachments: [],
      });
      ...
      harness.query.emit({
        ...
        uuid: "assistant-current-task-notification-result",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-current-task-notification-result",
          content: [{ type: "text", text: "Final task-notification answer" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-current-task-notification-result",
        uuid: "task-started-current-task-notification-result",
        task_id: "task-current-task-notification-result",
        tool_use_id: "tool-current-task-notification-result",
        description: "Run current turn command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-current-task-notification-result",
        uuid: "task-notification-current-task-notification-result",
        task_id: "task-current-task-notification-result",
        tool_use_id: "tool-current-task-notification-result",
        status: "completed",
        summary: "Current turn command completed",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        is_error: false,
        errors: [],
        result: "Final task-notification answer",
        stop_reason: "end_turn",
        session_id: "sdk-session-current-task-notification-result",
  ...
  it.effect("completes later detached turns from owned task-notification results", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
  ...
  it.effect("completes a detached fresh turn from the next SDK result immediately", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
  ...
  it.effect(
    "completes a foreground turn from an owned task-notification result immediately",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
  ...
  it.effect("completes a correlated task-notification result even with active tasks", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
  ...
  it.effect("completes the final owned task-notification result for a multi-task turn", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      const turn = yield* adapter.sendTurn({
        threadId: session.threadId,
        input: "wait for both owned tasks",
        attachments: [],
      });
      ...
      for (const task of ["first", "second"] as const) {
        harness.query.emit({
          type: "system",
          subtype: "task_started",
          session_id: "sdk-session-owned-active-tasks",
          uuid: `task-started-owned-active-${task}`,
          task_id: `task-owned-active-${task}`,
          tool_use_id: `tool-owned-active-${task}`,
          description: `Run ${task} foreground command`,
          task_type: "local_bash",
        } as unknown as SDKMessage);
      }
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-owned-active-tasks",
        uuid: "task-notification-owned-active-first",
        task_id: "task-owned-active-first",
        tool_use_id: "tool-owned-active-first",
        status: "completed",
        summary: "First foreground command completed",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-owned-active-tasks",
        uuid: "task-notification-owned-active-second",
        task_id: "task-owned-active-second",
        tool_use_id: "tool-owned-active-second",
        status: "completed",
        summary: "Second foreground command completed",
      } as unknown as SDKMessage);
      harness.query.emit({
        ...
        is_error: false,
        errors: [],
        result: "final owned foreground result",
        stop_reason: "end_turn",
        session_id: "sdk-session-owned-active-tasks",
        uuid: "result-owned-active-final",
        origin: { kind: "task-notification" },
      } as unknown as SDKMessage);
  ...
  it.effect(
    "does not complete from an uncorrelated task-notification result after a new task starts",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        const turn = yield* adapter.sendTurn({
          threadId: session.threadId,
          input: "keep running after a newer task starts",
          attachments: [],
        });
        ...
        harness.query.emit({
          ...
          uuid: "task-started-before-new-task",
          task_id: "task-before-new-task",
          tool_use_id: "tool-before-new-task",
        ...
        harness.query.emit({
          ...
          subtype: "task_notification",
          session_id: "sdk-session-task-result-after-new-task",
          uuid: "task-notification-before-new-task",
          task_id: "task-before-new-task",
          tool_use_id: "tool-before-new-task",
          status: "completed",
        ...
        harness.query.emit({
          ...
          subtype: "task_started",
          session_id: "sdk-session-task-result-after-new-task",
          uuid: "task-started-after-new-task",
          task_id: "task-after-new-task",
          tool_use_id: "tool-after-new-task",
        ...
        harness.query.emit({
          ...
          subtype: "task_notification",
          session_id: "sdk-session-task-result-after-new-task",
          uuid: "task-notification-after-new-task",
          task_id: "task-after-new-task",
          tool_use_id: "tool-after-new-task",
          status: "completed",
        ...
        harness.query.emit({
          ...
          is_error: false,
          errors: [],
          result: "real result after second task",
          stop_reason: "end_turn",
          session_id: "sdk-session-task-result-after-new-task",
          uuid: "result-real-after-new-task",
        } as unknown as SDKMessage);
  ...
  it.effect(
    "does not complete a detached turn from an uncorrelated task result after a newer task notification",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
  ...
  it.effect("does not flush a stale synthetic task completion into the next user turn", () => {
    ...
    return Effect.gen(function* () {
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-task-stale",
        uuid: "assistant-synthetic-task-stale",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-task-stale",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale",
        uuid: "task-started-stale",
        task_id: "task-stale",
        tool_use_id: "tool-stale",
        description: "Run stale background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      const userTurn = yield* adapter.sendTurn({
        threadId: session.threadId,
        input: "start the real user turn",
        attachments: [],
      });
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale",
        uuid: "task-started-current-after-stale-notification",
        task_id: "task-current-after-stale-notification",
        tool_use_id: "tool-current-after-stale-notification",
        description: "Run current user command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale",
        uuid: "task-notification-current-after-stale-notification",
        task_id: "task-current-after-stale-notification",
        tool_use_id: "tool-current-after-stale-notification",
        status: "completed",
        summary: "Current user command completed",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        result: "real user result",
        session_id: "sdk-session-task-stale",
        uuid: "result-current-after-stale-notification",
      } as unknown as SDKMessage);
  ...
  it.effect("does not let a stale task result consume the active task candidate", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "assistant-synthetic-task-stale-active-candidate",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-task-stale-active-candidate",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "task-started-stale-active-candidate",
        task_id: "task-stale-active-candidate",
        tool_use_id: "tool-stale-active-candidate",
        description: "Run stale background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "task-started-current-active-candidate",
        task_id: "task-current-active-candidate",
        tool_use_id: "tool-current-active-candidate",
        description: "Run current user command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "task-notification-current-active-candidate",
        task_id: "task-current-active-candidate",
        tool_use_id: "tool-current-active-candidate",
        status: "completed",
        summary: "Current user command completed",
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "task-notification-stale-active-candidate",
        task_id: "task-stale-active-candidate",
        tool_use_id: "tool-stale-active-candidate",
        status: "completed",
        summary: "Background command completed",
      ...
      harness.query.emit({
        ...
        is_error: false,
        errors: [],
        result: "stale synthetic result",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "result-stale-active-candidate",
        origin: { kind: "task-notification" },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        result: "real user result",
        session_id: "sdk-session-task-stale-active-candidate",
        uuid: "result-real-active-candidate",
      } as unknown as SDKMessage);
      ...
      assert.equal(userTurnCompletions.length, 1);
      const staleTaskCompletionsForUserTurn = runtimeEvents.filter(
        (event) => event.type === "task.completed" && event.turnId === userTurn.turnId,
      );
      assert.equal(staleTaskCompletionsForUserTurn.length, 1);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("carries stale synthetic task drains across the next user turn", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "assistant-synthetic-task-stale-carried-drain",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-task-stale-carried-drain",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "task-started-stale-carried-drain",
        task_id: "task-stale-carried-drain",
        tool_use_id: "tool-stale-carried-drain",
        description: "Run stale background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "task-notification-stale-carried-drain",
        task_id: "task-stale-carried-drain",
        tool_use_id: "tool-stale-carried-drain",
        status: "completed",
        summary: "Background command completed",
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "task-started-current-carried-drain",
        task_id: "task-current-carried-drain",
        tool_use_id: "tool-current-carried-drain",
        description: "Run current user command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "task-notification-current-carried-drain",
        task_id: "task-current-carried-drain",
        tool_use_id: "tool-current-carried-drain",
        status: "completed",
        summary: "Current user command completed",
      ...
      harness.query.emit({
        ...
        result: "stale synthetic result",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "result-stale-carried-drain",
        origin: { kind: "task-notification" },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        result: "real user result",
        session_id: "sdk-session-task-stale-carried-drain",
        uuid: "result-real-carried-drain",
      } as unknown as SDKMessage);
  ...
  it.effect("does not let a carried stale drain swallow an explicit current result", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-task-stale-explicit-current",
        uuid: "assistant-synthetic-task-stale-explicit-current",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-task-stale-explicit-current",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-explicit-current",
        uuid: "task-started-stale-explicit-current",
        task_id: "task-stale-explicit-current",
        tool_use_id: "tool-stale-explicit-current",
        description: "Run stale background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-explicit-current",
        uuid: "task-notification-stale-explicit-current",
        task_id: "task-stale-explicit-current",
        tool_use_id: "tool-stale-explicit-current",
        status: "completed",
        summary: "Background command completed",
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-explicit-current",
        uuid: "task-started-current-explicit-current",
        task_id: "task-current-explicit-current",
        tool_use_id: "tool-current-explicit-current",
        description: "Run current user command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-explicit-current",
        uuid: "task-notification-current-explicit-current",
        task_id: "task-current-explicit-current",
        tool_use_id: "tool-current-explicit-current",
        status: "completed",
        summary: "Current user command completed",
      ...
      harness.query.emit({
        ...
        is_error: false,
        errors: [],
        result: "explicit current result",
        session_id: "sdk-session-task-stale-explicit-current",
        uuid: "result-current-explicit-current",
        turnId: String(userTurn.turnId),
        origin: { kind: "task-notification" },
      } as unknown as SDKMessage);
  ...
  it.effect("completes the next user turn from the next SDK result", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-task-stale-defer",
        uuid: "assistant-synthetic-task-stale-defer",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-task-stale-defer",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-task-stale-defer",
        uuid: "task-started-stale-defer",
        task_id: "task-stale-defer",
        tool_use_id: "tool-stale-defer",
        description: "Run stale background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-task-stale-defer",
        uuid: "task-notification-stale-defer",
        task_id: "task-stale-defer",
        tool_use_id: "tool-stale-defer",
        status: "completed",
        summary: "Background command completed",
      ...
      harness.query.emit({
        ...
        result: "stale synthetic result",
        session_id: "sdk-session-task-stale-defer",
        uuid: "result-stale-synthetic-after-user-turn",
        origin: { kind: "task-notification" },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        result: "real user result",
        session_id: "sdk-session-task-stale-defer",
        uuid: "result-real-user-after-stale-task",
      } as unknown as SDKMessage);
      ...
      assert.equal(staleTaskCompletionsForUserTurn.length, 0);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect(
    "does not drop a real user result while a stale synthetic task result is pending",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        harness.query.emit({
          type: "assistant",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "assistant-synthetic-task-stale-real-first",
          parent_tool_use_id: null,
          message: {
            id: "assistant-message-synthetic-task-stale-real-first",
            content: [{ type: "text", text: "Background update" }],
          },
        } as unknown as SDKMessage);
        ...
        harness.query.emit({
          ...
          subtype: "task_started",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "task-started-stale-real-first",
          task_id: "task-stale-real-first",
          tool_use_id: "tool-stale-real-first",
          description: "Run stale background command",
          task_type: "local_bash",
        } as unknown as SDKMessage);
        ...
        harness.query.emit({
          ...
          subtype: "task_started",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "task-started-current-real-first",
          task_id: "task-current-real-first",
          tool_use_id: "tool-current-real-first",
          description: "Run current user command",
          task_type: "local_bash",
        } as unknown as SDKMessage);
        ...
        harness.query.emit({
          ...
          subtype: "task_notification",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "task-notification-current-real-first",
          task_id: "task-current-real-first",
          tool_use_id: "tool-current-real-first",
          status: "completed",
          summary: "Current user command completed",
        ...
        harness.query.emit({
          ...
          result: "real user result",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "result-real-user-before-stale-task-result",
        } as unknown as SDKMessage);
        ...
        harness.query.emit({
          ...
          subtype: "task_notification",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "task-notification-stale-real-first",
          task_id: "task-stale-real-first",
          tool_use_id: "tool-stale-real-first",
          status: "completed",
          summary: "Background command completed",
        ...
        harness.query.emit({
          ...
          result: "stale synthetic result",
          session_id: "sdk-session-task-stale-real-first",
          uuid: "result-stale-task-after-real-user",
          origin: { kind: "task-notification" },
        } as unknown as SDKMessage);
  ...
  it.effect("completes synthetic task results immediately", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-synthetic-task",
        uuid: "assistant-synthetic-task",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-task",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-synthetic-task",
        uuid: "task-started-synthetic",
        task_id: "task-synthetic",
        tool_use_id: "tool-synthetic",
        description: "Run synthetic background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-synthetic-task",
        uuid: "task-notification-synthetic",
        task_id: "task-synthetic",
        tool_use_id: "tool-synthetic",
        status: "completed",
        summary: "Background command completed",
      ...
      const syntheticTurn = runtimeEvents.find((event) => event.type === "turn.started");
      const completedTurns = runtimeEvents.filter((event) => event.type === "turn.completed");
      assert.equal(completedTurns.length, 1);
      assert.equal(completedTurns[0]?.turnId, syntheticTurn?.turnId);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("keeps task starts before assistant output on the synthetic turn", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "system",
        subtype: "task_started",
        session_id: "sdk-session-pre-assistant-task",
        uuid: "task-started-pre-assistant",
        task_id: "task-pre-assistant",
        tool_use_id: "tool-pre-assistant",
        description: "Run background command before assistant text",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-pre-assistant-task",
        uuid: "assistant-pre-assistant-task",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-pre-assistant-task",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        result: "FINAL",
        session_id: "sdk-session-pre-assistant-task",
        uuid: "result-pre-assistant-task",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_notification",
        session_id: "sdk-session-pre-assistant-task",
        uuid: "task-notification-pre-assistant",
        task_id: "task-pre-assistant",
        tool_use_id: "tool-pre-assistant",
        status: "completed",
        summary: "Background command completed",
      ...
      assert.equal(taskStarted?.turnId, syntheticTurn?.turnId);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("keeps an already completed synthetic result separate from the next user turn", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-synthetic-autoclose",
        uuid: "assistant-synthetic-autoclose",
        parent_tool_use_id: null,
        message: {
          id: "assistant-message-synthetic-autoclose",
          content: [{ type: "text", text: "Background update" }],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        subtype: "task_started",
        session_id: "sdk-session-synthetic-autoclose",
        uuid: "task-started-autoclose",
        task_id: "task-autoclose",
        tool_use_id: "tool-autoclose",
        description: "Run autoclose background command",
        task_type: "local_bash",
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        result: "FINAL",
        session_id: "sdk-session-synthetic-autoclose",
        uuid: "result-synthetic-autoclose",
      } as unknown as SDKMessage);
      ...
      const completedAfterSend = runtimeEvents.filter((event) => event.type === "turn.completed");
      assert.equal(completedAfterSend.length, 1);
      assert.notEqual(completedAfterSend[0]?.turnId, userTurn.turnId);

      harness.query.emit({
        type: "system",
        subtype: "task_notification",
        session_id: "sdk-session-synthetic-autoclose",
        uuid: "task-notification-autoclose",
        task_id: "task-autoclose",
        tool_use_id: "tool-autoclose",
        status: "completed",
        summary: "Background command completed",
      ...
      harness.query.emit({
        ...
        is_error: false,
        errors: [],
        result: "stale synthetic task result",
        session_id: "sdk-session-synthetic-autoclose",
        uuid: "result-stale-synthetic-autoclose-task",
        origin: { kind: "task-notification" },
      } as unknown as SDKMessage);
      ...
      assert.equal(userTurnCompletions.length, 0);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("treats Claude user-steer diagnostics as interrupted without a runtime error", () => {
    const cases = [
      {
        isError: false,
    ...
    return Effect.forEach(cases, (testCase, index) => {
      ...
      return Effect.gen(function* () {
        ...
        harness.query.emit({
          ...
          session_id: `sdk-session-abort-${index}`,
          uuid: `result-abort-${index}`,
        } as unknown as SDKMessage);
        ...
        if (turnCompleted?.type === "turn.completed") {
          ...
          assert.equal(turnCompleted.payload.stopReason, testCase.stopReason);
        }
  ...
  it.effect("keeps genuine Claude execution errors failed", () => {
    const cases = [
      {
        isError: true,
        error: "Claude execution failed while invoking a tool.",
        stopReason: "tool_use",
        terminalReason: undefined,
      },
      {
        isError: false,
        error: "[ede_diagnostic] result_type=user status=failed",
        stopReason: null,
        terminalReason: undefined,
      },
      {
    ...
    return Effect.forEach(cases, (testCase, index) => {
      ...
      return Effect.gen(function* () {
        ...
        if (runtimeError?.type === "runtime.error") {
          assert.equal(runtimeError.payload.message, testCase.error);
        }
        ...
        const sessions = yield* adapter.listSessions();
        assert.equal(sessions[0]?.status, "ready");
        assert.equal(sessions[0]?.lastError, testCase.error);
      }).pipe(
        Effect.provideService(Random.Random, makeDeterministicRandomService()),
        Effect.provide(harness.layer),
      );
  ...
  it.effect("exits normally after a promoted subagent wait when the Claude stream ends", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      assert.equal(waitingEvent, undefined);
  ...
  it.effect("exits compacting promoted waits without dropping result metadata", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
  ...
  it.effect("does not park promoted subagent waits on result completion", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "stream_event",
        session_id: "sdk-session-waiting-subagent-text",
        uuid: "stream-wait-subagent-text-start",
        parent_tool_use_id: null,
        event: {
          type: "content_block_start",
          index: 1,
      ...
      harness.query.emit({
        type: "stream_event",
        session_id: "sdk-session-waiting-subagent-text",
        uuid: "stream-wait-subagent-text-stop",
        parent_tool_use_id: null,
        event: {
          type: "content_block_stop",
          index: 1,
      ...
      harness.query.emit({
        type: "user",
        session_id: "sdk-session-waiting-subagent-text",
        uuid: "user-wait-subagent-text-result",
        parent_tool_use_id: null,
        tool_use_result: {
          content:
            '{"results":[{"childThreadId":"child-thread-1","status":"running"}],"promoted":true}',
        ...
        message: {
          ...
          content: [
            {
              type: "tool_result",
              tool_use_id: "tool-wait-subagent-text-1",
              content: 'promoted=true; [{"childThreadId":"child-thread-1","status":"running"}]',
            },
          ],
        },
      ...
      harness.query.emit({
        ...
        stop_reason: "end_turn",
        session_id: "sdk-session-waiting-subagent-text",
        uuid: "result-wait-subagent-text",
      } as unknown as SDKMessage);
  ...
  it.effect("does not park subagent waits when promoted only appears in unstructured text", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      ...
      harness.query.emit({
        type: "stream_event",
        session_id: "sdk-session-waiting-subagent-text-false-positive",
        uuid: "stream-wait-subagent-text-false-positive-start",
        parent_tool_use_id: null,
        event: {
          type: "content_block_start",
          index: 1,
          content_block: {
            type: "mcp_tool_use",
            id: "tool-wait-subagent-text-false-positive-1",
            name: "t3_wait_subagent",
            input: {
              childThreadIds: ["child-thread-1"],
            },
      ...
      harness.query.emit({
        type: "stream_event",
        session_id: "sdk-session-waiting-subagent-text-false-positive",
        uuid: "stream-wait-subagent-text-false-positive-stop",
        parent_tool_use_id: null,
        event: {
          type: "content_block_stop",
          index: 1,
      ...
      harness.query.emit({
        type: "user",
        session_id: "sdk-session-waiting-subagent-text-false-positive",
        uuid: "user-wait-subagent-false-positive-result",
        parent_tool_use_id: null,
        tool_use_result: {
          content:
            '{"results":[{"childThreadId":"child-thread-1","status":"completed","finalAssistantText":"child wrote promoted=true"}],"pending":false}',
          structuredContent: {
            results: [
              {
                childThreadId: "child-thread-1",
                status: "completed",
                finalAssistantText: "child wrote promoted=true",
              },
            ],
            pending: false,
          },
        ...
        message: {
          ...
          content: [
            {
              type: "tool_result",
              tool_use_id: "tool-wait-subagent-text-false-positive-1",
              content: "child wrote promoted=true in its final answer",
            },
          ],
        },
      } as unknown as SDKMessage);
      ...
      harness.query.emit({
        ...
        stop_reason: "end_turn",
        session_id: "sdk-session-waiting-subagent-text-false-positive",
        uuid: "result-wait-subagent-false-positive",
      } as unknown as SDKMessage);
      harness.query.finish();
