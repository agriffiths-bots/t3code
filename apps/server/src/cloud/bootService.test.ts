...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("refuses to stop or remove a foreign unit", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(plan.unitPath, foreignUnit);

      const error = yield* service.uninstall.pipe(Effect.flip);

      expect(error._tag).toBe("BootServiceOwnershipError");
      if (error._tag !== "BootServiceOwnershipError") return;
      expect(error.unitPath).toBe(plan.unitPath);
      expect(commands).toEqual([]);
      expect(yield* fs.readFileString(plan.unitPath)).toBe(foreignUnit);
    }),
  );
