...
/** Mixed-version stream guard: settlement union variants are only exposed to
 * clients that explicitly advertise decoder support. */
...
const makeWsRpcLayer = (currentSession: EnvironmentAuth.AuthenticatedSession) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const heapDiagnostics = yield* HeapDiagnostics.HeapDiagnostics;
      ...
      return WsRpcGroup.of({
        [ORCHESTRATION_WS_METHODS.dispatchCommand]: (command) =>
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              const dispatchAuthority = sessionDispatchAuthority(currentSession);
              ...
              if (parkingCommand) {
                ...
                if (shouldStopSessionAfterCommand) {
                  yield* Effect.gen(function* () {
                    const stopCommand = yield* normalizeAuthorizedDispatchCommand(
                      {
                      ...
                      if (Option.isSome(latestDeletion)) {
                        return Stream.concat(
                          Stream.make({
                            ...
                            storageEpoch,
                            force: true,
                            event: projectActivityEvent(latestDeletion.value),
                          }),
                          liveBufferStream,
                        );
                      }
        ...
        [WS_METHODS.serverWriteHeapSnapshot]: (input) =>
          observeRpcEffect(
            WS_METHODS.serverWriteHeapSnapshot,
            heapDiagnostics.writeSnapshot(input),
            {
              "rpc.aggregate": "server",
            },
          ),
        ...
        [WS_METHODS.reviewGetDiffFileContents]: (input) =>
          observeRpcEffect(
            ...
            ensureGitCwdVisible({
              ...
              command: "git diff",
              cwd: input.cwd,
            }).pipe(Effect.andThen(review.getDiffFileContents(input))),
            { "rpc.aggregate": "review" },
          ),
