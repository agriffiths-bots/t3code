...
export type SessionOnlySendReason = "thread-setup" | "dependent";
...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  if (isRunning && !hasSendableContent) {
    return (
      <button
        type="button"
        className={cn(
          "flex h-9 cursor-pointer items-center justify-center rounded-full bg-destructive/90 text-white shadow-xs shadow-destructive/24 inset-shadow-[0_1px_--theme(--color-white/16%)] transition-all duration-150 hover:bg-destructive hover:scale-105 active:inset-shadow-[0_1px_--theme(--color-black/8%)] active:shadow-none sm:h-8",
      ...
      >
        ...
        {showSendLabel ? <span>Stop</span> : null}
      </button>
    );
  }
  ...
  const disconnectedQueueLabel = hasImageAttachments
    ? "Queue attachments for this session"
    : sessionOnlySendReason === "thread-setup"
      ? "Queue thread setup for this session"
      : sessionOnlySendReason === "dependent"
        ? "Queue send after earlier queued work for this session"
        : "Queue text message";
