...
describe("resolveAssetUrl", () => {
  ...
  it("keeps old-server asset results on the environment origin", async () => {
    await expect(
      resolveBoundAssetUrl("https://environment.example/", "t3code://app/", {
        relativeUrl: "/api/assets/signed-token/legacy-report.html",
        expiresAt: Date.now() + 60_000,
      }),
    ).resolves.toBe("https://environment.example/api/assets/signed-token/legacy-report.html");
  });
