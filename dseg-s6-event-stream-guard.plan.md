...
| Version skew: v1 devices lack audience               | `migrates unattributed version-one notification devices fail-closed`                           |
...
| Global thread revision cursor                        | `restricted thread-revision HTTP reads refuse before loading aggregate or projection cursors`  |
...
| Notification delivery count                          | `factory notification result counts factory-visible devices without narrowing fan-out`         |
| Broad shared read scope                              | `factory tokens reach only guarded orchestration HTTP/RPC reads; diagnostics stay denied`      |
| Unrestricted compatibility                           | `private replay, streams, notifications, schedules, and attributed v1 devices remain complete` |

Smallest change: keep canonical stores, public RPC contracts, and client reducers unchanged; extend the existing audience scope policy with one explicit allowlist for guarded `orchestration:read` RPCs, add one shared projection-backed aggregate visibility query for safe schedule/notification reads, and use one common factory cursor refusal for replay, shell, and thread streams. The approved cursor-namespace follow-up owns event persistence and recovery semantics; this slice suppresses unchanged restricted schedule snapshots, rebases their session-local sequence densely, makes notification result counts caller-visible without narrowing authorized fan-out, and migrates v1 devices to required-at-rest v2 audience metadata while preserving explicit ceilings and assigning unattributed legacy devices the least-privileged ceiling.
